...
def initialize_i2c_sensors():
    ...
    try:
        ...
        # FIX: P0 removed from library, using raw integer 0 for Pin 0
        o2_channel = AnalogIn(ads, 0)
...
def update_lcd_display(stage, mode):
    ...
    if lcd:
        try:
            ...
            print(f"LCD I2C Communication Error: {e}")
            lcd = None # Prevent script from crashing on next loop
        ...
        while True:
            ...
            if current_mode == "AUTO":
                if not dht_error:
                    ...
                    # --- TRUE HYSTERESIS / DEADBAND LOGIC ---
                    if temperature <= auto_target_temp - 0.5:
                        # 1. TOO COLD (Hit the floor): Turn heaters ON
                        ...
                        # 2. TOO HOT (Hit the ceiling): Turn heaters OFF
                        dehumidifier_on, dehumidifier_on_2 = False, False
                        fan_on = True
                        
                    else:
                        # 3. INSIDE THE DEADBAND (e.g. between Target-0.5 and Target+0.5)
                        # Do NOT change the heater state! Just maintain whatever they are already doing.
                        
                        # If heaters are already running, keep checking the delay timer for Heat 2
                        if dehumidifier_on and (t - dehum_1_on_time > SECONDARY_ACTUATOR_DELAY): 
                            dehumidifier_on_2 = True
                            
                        # If heaters are currently OFF in the deadband, apply the gentle stage fan setting
                    ...
                    # --- OXYGEN TRIM CONTROL ---
                    if not o2_error:
                        target_servo = servo_angle # Default to current state
                        if o2_voltage < 0.05:
                            pass # Sensor is still cold. Do nothing.
                        elif o2_voltage > 0.70:
                            target_servo = 180 # Rich burn (starving): open fully
                        elif o2_voltage < 0.20:
                            target_servo = 45  # Lean burn (wasting heat): close down
                        else:
                            target_servo = 90  # Good burn: hold steady
                            
                        # Only command the servo if the target changed so we don't spam it!
                        if target_servo != servo_angle:
                            set_servo_angle(target_servo)
                            servo_angle = target_servo
                            save_state()
                    # ---------------------------
