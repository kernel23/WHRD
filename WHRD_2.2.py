...
def main():
    ...
    try:
        while True:
            ...
            if current_mode == "AUTO":
                if not dht_error:
                    # --- GHOST DATA SANITY CHECK ---
                    if stage_start_temp < setpoints["min_temp"]:
                        stage_start_temp = max(temperature, setpoints["min_temp"])
                    ...
                    if temperature <= auto_target_temp - 0.5:
                        ...
                        # Only handle the stage-default fan setting if heaters are currently off
